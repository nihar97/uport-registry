...
contract UportRegistry {
  ...
  function getOne(bytes32 registrationIdentifier, address issuer, address subject, uint index) constant returns(bytes32) {
    return registry[registrationIdentifier][issuer][subject][index];
  }

  // function update(bytes32 registrationIdentifier, address subject, bytes32 value, uint index) {
  //   registry[registrationIdentifier][msg.sender][subject][index] = value;
  // }
}
